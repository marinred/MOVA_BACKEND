...
class ColorizationView(APIView):
    def post(self, request):
        serializer = FanartImageSerializer(
            data=request.data)  # resize_image, hint_image 불러와서
        print(request.data)
        if serializer.is_valid():
            serializer.save()  # resize_image, hint_image 저장
            # colorization 함수 실행을 위해 resize_image 이름 가져옴
            resize_image = BaseImage.objects.get(
                id=serializer.data['resize_image']).image.url[1:]
            # colorization 함수 실행을 위해 hint_image 이름 가져옴
            hint_image = serializer.data['hint_image'][1:]
...
class FanartListView(APIView):
    ...
    def get(self, request):
        ...
        fanart_likes = Fanart.objects.annotate(
